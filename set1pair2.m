...
imagev2 = rgb2gray(im2double(imread('plasticClip3.jpg')));

imagev2 = imresize(imagev2,0.4);
imshow(imagev2);
%resizedimage = imresize(image,[456 1000]);
%imshow(resizedimage);
...
highcorv2 = cornernessv2.*(cornernessv2 > (1e-6)*4); %the bigger num the less corner it shows eg 1e-5  (had(1e-5)*6 before as setting)
...
colorImage = imresize(colorImage,0.4);

colorImage2 = im2double(imread('plasticClip3.jpg'));

colorImage2 = imresize(colorImage2,0.4);
...
for i=1:length(keypointDescPool)
   for j=1:length(keypointDescPool2)
     ...
     if((rounded1(1,i)+0.1==rounded2(1,j))||(rounded1(1,i)-0.1==rounded2(1,j))||(rounded1(1,i)==rounded2(1,j))||(rounded1(1,i)==rounded2(1,j)+0.2)||(rounded1(1,i)==rounded2(1,j)-0.2))
