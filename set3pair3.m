...
image = rgb2gray(im2double(imread('myface1.jpg')));
...
imagev2 = rgb2gray(im2double(imread('myface4.jpg')));

imagev2 = imresize(imagev2,0.5);
imagev2 = imrotate(imagev2,180);
imshow(imagev2);
%resizedimage = imresize(image,[456 1000]);
%imshow(resizedimage);
...
%ixv2tt = imfilter(imagev2, ixv2,'replicate');
iyv2 = imfilter(imagev2, deriv_gaus_yv2,'replicate');
%iyv2tt = imfilter(imagev2, iyv2,'replicate');
...
highcorv2 = cornernessv2.*(cornernessv2 > (1e-6)*6); %the bigger num the less corner it shows eg 1e-5  (had(1e-5)*6 before as setting)
...
%cutting out the edge located corners as when extracting the 16by16 window,
%it may run out of bounds or not in the image array
for i = length(pointYv2):-1:1
    if( img2vertSize-50 <= pointYv2(i))
        pointYv2(i)= [];
        pointXv2(i)= [];
    end
...
colorImage = im2double(imread('myface1.jpg'));
colorImage = imrotate(colorImage,180);
colorImage = imresize(colorImage,0.5);

colorImage2 = im2double(imread('myface4.jpg'));
colorImage2 = imrotate(colorImage2,180);
...
%          if((ismember(tempa,temparray))&& (numel(find(temparray==tempa)))<5)
