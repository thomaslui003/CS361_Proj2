...
image = rgb2gray(im2double(imread('battery1.jpg')));
...
highcor = cornerness.*(cornerness > (1e-6)*8); %the bigger num the less corner it shows eg 1e-5
...
imagev2 = rgb2gray(im2double(imread('battery2.jpg')));
...
highcorv2 = cornernessv2.*(cornernessv2 > (1e-5)*3); %the bigger num the less corner it shows eg 1e-5  (had(1e-5)*6 before as setting)
...
colorImage = im2double(imread('battery1.jpg'));

colorImage = imresize(colorImage,0.5);

colorImage2 = im2double(imread('battery2.jpg'));
...
for i=1:length(keypointDescPool)
   for j=1:length(keypointDescPool2)
     ...
     %if((rounded1(1,i)+0.02==rounded2(1,j))||(rounded1(1,i)-0.02==rounded2(1,j))||(rounded1(1,i)==rounded2(1,j)))
